resource "null_resource" "get_target_group_arn" {
  provisioner "local-exec" {
    command = "aws elbv2 describe-target-groups --load-balancer-arn ${module.base.aws_elastic_beanstalk_environment_load_balancers[0]} --output text --query 'TargetGroups[?LoadBalancerArns==[`${module.base.aws_elastic_beanstalk_environment_load_balancers[0]}`]].TargetGroupArn'  > ${data.template_file.target_group_in_template.rendered}"
  }
